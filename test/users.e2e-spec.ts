...
describe('UserController (e2e)', () => {
  ...
  it('/:id (GET) [Empty]', () => request(app.getHttpServer())
  ...
  it('/ (POST) [Error]', () => request(app.getHttpServer())
    .post('/users')
    .send(dto)
    .expect(400)
    .then((res) => {
  ...
  it('/:id (PATCH) [Duplicate]', () => request(app.getHttpServer())
  ...
  it('/:id (GET)', () => request(app.getHttpServer())
    .get('/users/1') // the user previously created
    .expect(200)
    .then((res) => {
      expect(res.body).toHaveProperty('id');
  ...
  it('/:id (PATCH)', () => request(app.getHttpServer())
    .patch('/users/1') // the user previously created
    .send({ password: "aa" })
    .expect(200)
    .then((res) => {
  ...
  it('/:id (DELETE)', () => request(app.getHttpServer())
    .delete('/users/1') // the user previously created
    .expect(200)
  );

  it('/ (GET) [Empty]', () => request(app.getHttpServer())
    .get('/users')
    .expect(200)
    .expect([])
  );

  it('/:id (PATCH) [Not Existing]', () => request(app.getHttpServer())
    .patch('/users/1') // the user previously created
    .send({ password: "aa" })
    .expect(404)
    .then((res) => {
